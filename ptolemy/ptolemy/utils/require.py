...
def require(
    *packages: str, required_extras: List[str]
    ...
    def decorator(func) -> Callable[..., Any]:
        ...
        def wrapper(*args, **kwargs) -> Any:
            try:
                ...
                raise ImportError(
                    f"This functionality requires the following extras to be installed: {required_extras}. To install, run `pip install ptolemy[{','.join(required_extras)}]` or `poetry install -E {' -E '.join(required_extras)}"
                ) from e
