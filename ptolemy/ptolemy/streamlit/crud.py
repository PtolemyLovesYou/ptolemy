...
async def get_tier_event_names():
    async with session.get_db() as db:
        result = await db.execute(
            select(models.DB_OBJ_MAP[LogType.EVENT][st.session_state["filter.tier"]].name.distinct())
        )
...
async def get_tier_environments():
    async with session.get_db() as db:
        result = await db.execute(
            select(models.DB_OBJ_MAP[LogType.EVENT][st.session_state["filter.tier"]].environment.distinct())
        )
...
async def get_tier_versions():
    async with session.get_db() as db:
        result = await db.execute(
            select(models.DB_OBJ_MAP[LogType.EVENT][st.session_state["filter.tier"]].version.distinct())
        )
...
async def run_sql_query(query: str):
    ...
    return pd.DataFrame(
        ({c: getattr(r, c) for c in keys} for r in result.fetchall()),
        columns=keys
        )
...
async def get_event_objects():
    ...
    return pd.DataFrame(
        [
            {
                c: getattr(r, c) for c in ["id", "name", "version", "environment", "created_at"]
...
async def get_event_parameters():
    ...
    async with session.get_db() as db:
        result = await db.execute(
            select(model)
            .where(model.id == st.session_state["event_id"])
        )
...
async def get_event_io(log_type: LogType):
    ...
    async with session.get_db() as db:
        result = await db.execute(
            select(model)
            .where(model.parent_id == st.session_state["event_id"])
        )
...
async def get_event_details() -> dict:
    ...
    async with session.get_db() as db:
        event_result = await db.execute(
            select(event_model)
            .where(event_model.id == st.session_state["event_id"])
        )

        runtime_result = await db.execute(
            select(runtime_model)
            .where(runtime_model.parent_id == st.session_state["event_id"])
        )
