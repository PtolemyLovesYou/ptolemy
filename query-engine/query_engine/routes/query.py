...
from ..models.query import CreateQueryRequest, CreateQueryResponse, GetQueryResponse
...
@router.post("/new")
def new_query(
    data: CreateQueryRequest, state: AppState = Depends(get_state)
) -> CreateQueryResponse:
    """Create new query."""
    ...
    return CreateQueryResponse(query_id=executor.query_id)
...
def get_query(query_id: str, state: AppState = Depends(get_state)) -> GetQueryResponse:
    ...
    return GetQueryResponse(query_id=query_id, status=status)
