...
use crate::{state::PtolemyConfig, error::ApiError};
...
pub trait Sink: std::fmt::Debug + Send + Sync {
    ...
    ) -> Result<Self, ApiError> where Self: Sized;
...
impl SinkRegistry {
    ...
    pub async fn fanout(&self, messages: Vec<Record>) -> Vec<Result<(), ApiError>> {
        let futures = self.sinks.values().map(|sink| sink.send_batch(messages.clone()));
