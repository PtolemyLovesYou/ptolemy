...
try:
    sys = client.trace(
        ...
        version="1.2.3",
        environment="PROD",
        parameters={"model": "gpt-4", "service": "openai"},
    )

    with sys:
        # Log system-level metadata (strings only)
        sys.metadata(customer_id="123", session_id="789", request_id="req_456")
        ...
        with retriever:
            ...
            retriever.outputs(documents=documents, retrieval_time_ms=150)

            # Log any retrieval-specific metadata (strings only)
            retriever.metadata(cache_hit="false", index_version="v2.1")

        # Log the main system outputs
        completion_content = "Quantum computing is a type of computation..."
        sys.outputs(response=completion_content, documents=documents, token_count=525)
        ...
        sys.feedback(relevance_score=0.95, toxicity_score=0.02, response_quality=0.89)
