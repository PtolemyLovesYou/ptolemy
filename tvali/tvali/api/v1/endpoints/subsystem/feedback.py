...
async def get_subsystem_feedback(subsystem_feedback_id: str) -> SubsystemFeedback:
    ...
    event = (
        db
        .query(models.SubsystemFeedback)
        .filter(models.SubsystemEvent.id == subsystem_feedback_id)
        .first()
        )
...
async def delete_subsystem_feedback(subsystem_feedback_id: str):
    ...
    db = SessionLocal()
    db.query(models.SubsystemFeedback).filter(models.SubsystemEvent.id == subsystem_feedback_id).delete()
    db.commit()
